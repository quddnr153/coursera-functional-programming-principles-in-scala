...
object Huffman {
   ...
   *   }
   */
  def times(chars: List[Char]): List[(Char, Int)] = chars.groupBy(char => char).map(char => (char._1, char._2.size)).toList
   ...
   */
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sortWith(_._2 < _._2).map(freq => Leaf(freq._1, freq._2))
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case Nil => trees
    case head :: Nil => trees
    case first :: second :: tail => (makeCodeTree(first, second) :: tail).sortWith((tree1, tree2) => weight(tree1) < weight(tree2))
  }
